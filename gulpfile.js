...
const option = process.argv.includes('--dev') ? '--dev' : (process.argv.includes('--css') ? '--css' : undefined)
...
let initialTasks
if (option === '--dev') {
  initialTasks = series(compileScssDev, compilePug)
} else if (option === '--css') {
  initialTasks = compilePug
} else {
  initialTasks = series(comb, compileScss, compileScssMin, compilePug)
}
...
// Дефолтна задача
export default series(initialTasks, watchFiles)
